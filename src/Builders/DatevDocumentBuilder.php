...
use CKonverter\Core\Entities\Datev\{DatevDocument, DatevMetaHeaderLine, DatevFieldHeaderLine};
use CKonverter\Core\Enums\DatevFormat;
use CommonToolkit\Entities\Common\CSV\CSVHeaderField;
use RuntimeException;

final class DatevDocumentBuilder extends CSVDocumentBuilder {
    private DatevFormat $format = DatevFormat::Buchungsstapel;
    private ?DatevMetaHeaderLine $metaHeader = null;


    public function __construct(DatevFormat $format = DatevFormat::Buchungsstapel, string $delimiter = ';', string $enclosure = '"') {
        parent::__construct($delimiter, $enclosure);
        $this->format = $format;
    }

    public function setMetaHeader(DatevMetaHeaderLine $header): self {
     ...
     */
    protected function createFixedFieldHeader(): DatevFieldHeaderLine {
        $fields = [
